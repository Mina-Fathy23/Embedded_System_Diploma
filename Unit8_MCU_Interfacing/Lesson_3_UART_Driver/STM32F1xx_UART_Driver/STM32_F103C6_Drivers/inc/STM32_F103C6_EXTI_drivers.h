...
typedef struct
{
	EXTI_GPIO_Mapping_t EXTI_PIN;		//Specifies the External interrupt GPIO Mapping
										//This Parameter must be passed on @ref EXTI_define

	uint8				Trigger_Case;	//Specifies Rising, Falling or Both Triggers
										//This Parameter must be passed on @ref EXTI_Trigger_define

	uint8				IRQ_EN;			//Enable or Disable EXTI IQR (That will Enable the IRQ MASK and also on the NVIC interrupt controller)
										//This Parameter must be passed on @ref EXTI_IRQ_define
